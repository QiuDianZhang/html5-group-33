...
1. 计算当月有多少天（daysInMonth）及当月1号是周几（firstDayOfWeek）。
2. 生成 firstDayOfWeek 个空白格子占位。
3. 循环 1 到 daysInMonth，构建日期 Key（如 2025-12-01）。
4. 用 Key 去 reading_duration 中查询时长。
5. 根据时长设定 CSS 等级类名（level-1, level-2, level-3）渲染颜色。
